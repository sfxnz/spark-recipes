...
| Qwen3.8-Flash-Next-NVFP4 | 2× Spark, TP=2 | vLLM · `vllm/vllm-openai:nightly-aarch64@sha256:df871f17…` plus `docker/ple_layer.py` and `docker/modelopt.py` · snapshot `fab0aec` | ModelOpt NVFP4 W4A4 experts, FP8 PLE, MTP FP8_BLOCK_SCALES | 262,144 | 43.2 | 36.2 (agg 68.2) | MTP-3 | Vision wired (`image_url`, `smoke_vision.py`). Thinking off by default (`enable_thinking=false`). Occupancy is eight sequences. Native window. 1M is a lab ceiling, not a trained window. `run.sh` refuses a window above 1,048,576, a window above 262,144 without `VLLM_ALLOW_LONG_MAX_MODEL_LEN=1`, seqs above 8, global `marlin`, and `flashinfer_cutlass`. | 2026-09-05 | [repo](https://github.com/sfxnz/Qwen3.8-Flash-Next-NVFP4-vLLM-2x-DGX-Spark) |

...
| Qwen3.8-Flash-Next-NVFP4 (~180B MoE, 512 experts top-10, native ViT) | 2× Spark, TP=2 | vLLM · `vllm/vllm-openai:qwen38-flash-next@sha256:3b0e188f…` plus `docker/ple_layer.py` overlay · snapshot `7b719225` | ModelOpt NVFP4 W4A4 experts, FP8 PLE | 1,048,576 (native 262,144) | — | — | MTP-3 | Boot-verified only. Decode is not tabled. Thinking off by default (`enable_thinking=false`). `GET /health` 200 and `/v1/models` lists `RadixArk/Qwen3.8-Flash-Next-NVFP4`. Auto MoE selected `FLASHINFER_CUTLASS`. `run.sh` refuses a window above 1,048,576, seqs above 2, global `marlin` (unquantized MTP rejects it), and `flashinfer_cutlass`. 1M is a lab ceiling, not a trained window. | 2026-09-04 | [repo](https://github.com/sfxnz/Qwen3.8-Flash-Next-NVFP4-vLLM-2x-DGX-Spark) |
...
- **Decode bench (2× Spark recipes).** `bench_decode.py` is frozen in each repo. Decode only, streamed greedy, thinking off, 200 completion tokens, 3-run median, concurrency 1 and 2 (`python3 bench_decode.py` runs both phases at c=1,2). Only prose decode (free text) is published; the structured phase stays in the raw `bench.txt` evidence and is not tabled here or in the recipe READMEs. Tables report median per-stream decode tok/s, aggregate tok/s, and TTFT p50, run against the repo's default `run.sh` settings.
